...
function loadStand(name, partKey) {
    ...
    // FIX: If the user explicitly typed a path like "images/something.png" in StandStorage,
    // we don't want to accidentally double-prefix it into "images_extras/images/something.png"
    let finalImgSrc = stand.image.includes('/') ? stand.image : folderPrefix + stand.image;
    let finalStatsSrc = stand.stats.includes('/') ? stand.stats : folderPrefix + stand.stats;
    ...
    // Dynamic routing for the back button destination
    if (partKey === 'extras') {
